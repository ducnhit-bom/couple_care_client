...
1. **Parallel Subagent Research**:
   - Spawn up to **2 researcher subagents** (using `Task` tool with `Explore` subagent type) to investigate different aspects of the topic
   - Each subagent should focus on a different keyword/aspect for comprehensive coverage
   - Synthesize findings from all subagents into a cohesive report

2. **Search Strategy with each agent**:
   - Use `WebSearch` tool to search for relevant information
   - Run multiple `WebSearch` tools in parallel to gather comprehensive information
...
3. **Deep Content Analysis**:
   - When you found a potential GitHub repository URL, use `WebFetch` tool to read it
...
1. Be saved to `.claude/current-work/[folder-name]/research-report.md` if working within a `current-work` context
